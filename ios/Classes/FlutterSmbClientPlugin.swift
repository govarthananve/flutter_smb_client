...
class SMBConnection {
    ...
    func authenticate(username: String, password: String, domain: String) throws {
        ...
        connection?.send(
            content: negotiateCommand,
            completion: .contentProcessed { [weak self] error in
                if let error = error {
                    print("Negotiate request failed with detailed error: \(error)")
                    authError = SMBError.communicationError(
                        "Negotiate request failed: \(error.localizedDescription)")
                    semaphore.signal()
                    return
                }
                ...
                self?.connection?.receive(minimumIncompleteLength: 1, maximumLength: 65536) {
                    ...
                    if let error = error {
                        ...
                        authError = SMBError.communicationError(
                            "Failed to receive negotiate response: \(error.localizedDescription)")
                        semaphore.signal()
                        return
                    }
                    ...
                    self?.connection?.send(
                        content: setupCommand ?? Data(),
                        completion: .contentProcessed { error in
                            if let error = error {
                                print("Session setup request failed: \(error)")
                                authError = SMBError.communicationError(
                                    "Session setup request failed: \(error.localizedDescription)")
                                semaphore.signal()
                                return
                            }
                            ...
                            self?.connection?.receive(
                                ...
                                if let error = error {
                                    ...
                                    authError = SMBError.communicationError(
                                        "Failed to receive session setup response: \(error.localizedDescription)"
                                    )
                                    semaphore.signal()
                                    return
                                }
                                ...
                                if let setupData = content {
                                    print(
                                        "Received session setup response: \(setupData.count) bytes")
                                    if setupData.count >= 44 {
                                        self?.sessionId = setupData[40...47].withUnsafeBytes {
                                            $0.load(as: UInt64.self)
                                        }
                                        print(
                                            "Session established with ID: \(self?.sessionId ?? 0)")

                                        self?.establishTreeConnection { error in
                                            if let error = error {
                                                print("Tree connection failed: \(error)")
                                                authError = SMBError.communicationError(
                                                    "Tree connection failed: \(error.localizedDescription)"
                                                )
                                            }
                                            semaphore.signal()
                                        }
                                        ...
                                        authError = SMBError.invalidData(
                                            "Invalid session setup response length")
                                        semaphore.signal()
                                    }
                                } else {
                                    print("No session setup response received")
                                    authError = SMBError.communicationError(
                                        "No session setup response received")
                                    semaphore.signal()
                                }
                            }
                        })
    ...
    private func establishTreeConnection(completion: @escaping (Error?) -> Void) {
        ...
        connection?.send(
            ...
            completion: .contentProcessed { [weak self] error in
                if let error = error {
                    completion(
                        SMBError.communicationError(
                            "Failed to send tree connect: \(error.localizedDescription)"))
                    return
                }
                ...
                self?.connection?.receive(minimumIncompleteLength: 1, maximumLength: 65536) {
                    ...
                    if let error = error {
                        completion(
                            SMBError.communicationError(
                                "Failed to receive tree connect response: \(error.localizedDescription)"
                            ))
                        return
                    }
    ...
    private func createSMB2SessionSetupCommand(username: String, password: String, domain: String)
        -> Data
    {
        var command = Data()
        ...
        connection?.send(
            ...
            completion: .contentProcessed { [weak self] error in
                if let error = error {
                    ...
                    listError = SMBError.communicationError(
                        "Failed to send enum request: \(error.localizedDescription)")
                    semaphore.signal()
                    return
                }
                ...
                self?.connection?.receive(minimumIncompleteLength: 1, maximumLength: 65536) {
                    ...
                    if let error = error {
                        ...
                        listError = SMBError.communicationError(
                            "Failed to receive response: \(error.localizedDescription)")
                        return
                    }
        ...
        connection?.send(
            ...
            completion: .contentProcessed { [weak self] error in
                if let error = error {
                    ...
                    listError = SMBError.communicationError(
                        "Failed to send list command: \(error.localizedDescription)")
                    semaphore.signal()
                    return
                }
                ...
                self?.connection?.receive(minimumIncompleteLength: 1, maximumLength: 65536) {
                    ...
                    if let error = error {
                        ...
                        listError = SMBError.communicationError(
                            "Failed to receive list response: \(error.localizedDescription)")
                        return
                    }
    ...
    private func parseSMB2SharesResponse(data: Data) -> [[String: Any]] {
        ...
        while offset < data.count {
            ...
            let nameLength = data[offset..<(offset + 2)].withUnsafeBytes {
            ...
            if let shareName = String(
                data: data[offset..<(offset + Int(nameLength))], encoding: .utf16LittleEndian)
            {
                shares.append([
                    "name": shareName,
                    "isDirectory": true,
    ...
    private func parseSMB2ListResponse(data: Data) -> [[String: Any]] {
        ...
        while offset < data.count {
            ...
            // Extract file info
            let nextOffset = data[offset..<(offset + 4)].withUnsafeBytes {
                $0.load(as: UInt32.self)
            }
            let fileNameLength = data[(offset + 60)..<(offset + 62)].withUnsafeBytes {
                $0.load(as: UInt16.self)
            }
            let fileAttributes = data[(offset + 56)..<(offset + 60)].withUnsafeBytes {
                $0.load(as: UInt32.self)
            }
            ...
            if let fileName = String(
                data: data[nameOffset..<(nameOffset + Int(fileNameLength))],
...
public class FlutterSmbClientPlugin: NSObject, FlutterPlugin {
    ...
    public static func register(with registrar: FlutterPluginRegistrar) {
        let channel = FlutterMethodChannel(
            name: "flutter_smb_client", binaryMessenger: registrar.messenger())
    ...
    private func handleConnect(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        let connectionId =
            (args["connectionId"] as? String) ?? FlutterSmbClientPlugin.defaultConnectionId
    ...
    private func handleDownloadFile(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        guard let args = call.arguments as? [String: Any],
            ...
            let toPath = args["toPath"] as? String
        else {
            result(
                FlutterError(
                    code: "INVALID_ARGUMENTS",
